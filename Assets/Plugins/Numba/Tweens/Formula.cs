...
namespace Numba.Tweens
{
    public abstract class Formula
    {
        ...
        public byte Calculate(byte from, byte to, float value) => Linear.Calculate(from, to, Remap(value));

        public sbyte Calculate(sbyte from, sbyte to, float value) => Linear.Calculate(from, to, Remap(value));

        public bool Calculate(bool from, bool to, float value) => Linear.Calculate(from, to, Remap(value));

        public char Calculate(char from, char to, float value) => Linear.Calculate(from, to, Remap(value));

        public short Calculate(short from, short to, float value) => Linear.Calculate(from, to, Remap(value));

        public ushort Calculate(ushort from, ushort to, float value) => Linear.Calculate(from, to, Remap(value));

        public int Calculate(int from, int to, float value) => Linear.Calculate(from, to, Remap(value));

        public uint Calculate(uint from, uint to, float value) => Linear.Calculate(from, to, Remap(value));

        public float Calculate(float from, float to, float value) => Linear.Calculate(from, to, Remap(value));

        public long Calculate(long from, long to, float value) => Linear.Calculate(from, to, Remap(value));

        public ulong Calculate(ulong from, ulong to, float value) => Linear.Calculate(from, to, Remap(value));

        public double Calculate(double from, double to, float value) => Linear.Calculate(from, to, Remap(value));

        public decimal Calculate(decimal from, decimal to, float value) => Linear.Calculate(from, to, Remap(value));

        public Vector2 Calculate(Vector2 from, Vector2 to, float value) => Linear.Calculate(from, to, Remap(value));

        public Vector3 Calculate(Vector3 from, Vector3 to, float value) => Linear.Calculate(from, to, Remap(value));

        public Vector4 Calculate(Vector4 from, Vector4 to, float value) => Linear.Calculate(from, to, Remap(value));

        public Quaternion Calculate(Quaternion from, Quaternion to, float value) => Linear.Calculate(from, to, Remap(value));

        public Rect Calculate(Rect from, Rect to, float value) => Linear.Calculate(from, to, Remap(value));

        public Matrix4x4 Calculate(Matrix4x4 from, Matrix4x4 to, float value) => Linear.Calculate(from, to, Remap(value));

        public Bounds Calculate(Bounds from, Bounds to, float value) => Linear.Calculate(from, to, Remap(value));

        public Color Calculate(Color from, Color to, float value) => Linear.Calculate(from, to, Remap(value));

        public Color32 Calculate(Color32 from, Color32 to, float value) => Linear.Calculate(from, to, Remap(value));
    }
}